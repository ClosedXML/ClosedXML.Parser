...
public class R1C1ReferenceTokenTests
{
    ...
    public static IEnumerable<object[]> TestDataOneCorner
    {
        get
        {
            ...
            {
                "R[7]C2",
                new ReferenceSymbol(new RowCol(ReferenceAxisType.Absolute, 2, ReferenceAxisType.Relative, 7))
            };

            yield return new object[]
            {
                "R812C[7]",
                new ReferenceSymbol(new RowCol(ReferenceAxisType.Relative, 7, ReferenceAxisType.Absolute, 812))
            };
        }
    }
